...
| **Orchestration** | **LangChain** (Python) | Handles loaders, text splitting, retriever chains, memory, condense-question pattern out of the box. | $0 |
...
| **Backend** | **FastAPI** (Python) | Free, async, auto docs (Swagger). | $0 |
| **Frontend** | **React** (Vite) | Free, standard, widely adopted. | $0 |
| **Database** | **MySQL** (local via Docker, or PlanetScale/Aiven/Railway free tier for a hosted demo) | Structured relational schema for queries, feedback, and eval runs. SQLite as an even simpler local-dev fallback if preferred. | $0 |
...
| **Version control / portfolio** | **GitHub** (public repo) | Standard source hosting and portfolio visibility. | $0 |
...
- **MySQL vs. SQLite:** SQLite is zero-setup and totally free. Recommendation: use MySQL via a free local Docker container for development, and
